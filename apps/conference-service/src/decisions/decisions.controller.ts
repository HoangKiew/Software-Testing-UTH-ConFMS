...
export class DecisionsController {
  ...
  async getSummary(@Param('id') conferenceId: string, @CurrentUser('userId') chairId: number) {
    return this.decisionsService.getSummaryForConference(conferenceId, chairId);
  }

  @Post('single')
  @Roles(RoleName.CHAIR)
  async makeDecision(@Body() dto: MakeDecisionDto, @CurrentUser('userId') chairId: number) {
    return this.decisionsService.makeDecision(dto, chairId);
  }

  @Post('bulk')
  @Roles(RoleName.CHAIR)
  async makeBulkDecisions(@Body() dto: BulkDecisionDto, @CurrentUser('userId') chairId: number) {
    return this.decisionsService.makeBulkDecisions(dto, chairId);
  }

  @Post('conference/:id/make-all')
  @Roles(RoleName.CHAIR)
  async makeAllDecisions(@Param('id') conferenceId: string, @CurrentUser('userId') chairId: number) {
